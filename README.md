...
```mermaid
flowchart LR
    IoT1(fa:fa-lightbulb <br/>IoT device) -->|Publish| broker(Mqtt broker)
    IoT2(fa:fa-plug <br/> IoT device) -->|Publish| broker
    IoT3(fa:fa-temperature-half <br/> IoT device) -->|Publish| broker
...
MqttBroker:
  - Name: "My broker"
    ClientPrefix: "mqtt2otel-myclient"
    Endpoint:
      Port: 32007
...
* **MQTT Broker**:
  ...
  * `ClientPrefix`: Prefix for the client ID when connecting to the broker. If omitted, the default value `mqtt2otel`
    is used. This prefix can be used to identify the connectin at the broker when debugging connection issues.
...
To extract the temperature, use the [JSONPath](https://www.rfc-editor.org/rfc/rfc9535) syntax `$.Processor.Temperature`. The corresponding YAML would look like this:
...
This configuration subscribes to the MQTT topic and creates an Otel metric called `Processor.Temperature` with a `float` data type and a `Gauge` instrument. Every time a message is received for the `message-topic`, the temperature value is sent to the Otel endpoint.
...
Otel rules can also include attributes, which are added to the Otel signal for filtering or grouping. Here’s an example of how to add attributes:
...
The attributes directly added under the otel section will be added to all rules. The attributes added to the "Processor.Temperature" rule
will only be added to this rule. 
...
Log messages work similarly to metrics. Let's say you receive a log message like this from MQTT:

```
2026-02-26T10:28:34 [Info] [ServerA] Temperature value read successfully.
```

Rather than sending the raw message to Otel, we can transform it into a structured log format using a [GROK](https://www.elastic.co/docs/reference/logstash/plugins/plugins-filters-grok) expression.
...
The `Transform` expression will convert the log into a JSON structure like this:

```json
{
  "otel_timestamp": "2026-02-26T10:28:34",
  "otel_loglevel": "Info",
  "server_name": "ServerA",
...
To avoid repetition and reuse the same subscriptions across different rules, you can group them into **Subscription Groups** and refer to them later. This is useful when you have multiple devices or sensors sending data under the same topic structure but need to handle them differently in your rules.

### Example Scenario

Let’s say you have a device that sends both power consumption metrics (like current, power, voltage) and status information (like the microcontroller core temperature) in the same MQTT message, structured as follows:
...
Here, we define a **Subscription Group** called `Power sensors`, which includes two subscriptions: one for a washing machine and another for a dryer. Both subscriptions have associated variables that can be used later in the rules.

### Using Subscription Groups in Metrics and Logs

Once you’ve created the `Power sensors` group, you can refer to it in your rules for **metrics** or **logs** as follows:
...
* **Power Metrics**: We create a metric for power data (e.g., `Power`), subscribing to the `Power sensors` group.
